...
public class StudyJoinRejectedEventHandler implements NotificationEventHandler<StudyJoinResultEventPayloadWithNotification> {

    private static final String JOIN_APPROVED_MESSAGE_TITLE_CODE = "notification.study.join-rejected.title";
    private static final String JOIN_APPROVED_MESSAGE_CONTENT_CODE = "notification.study.join-rejected.content";
    ...
    public NotificationDetails prepareDetails(EventType type, StudyJoinResultEventPayloadWithNotification payload) {
        ...
        StudyJoinResultPayload dataPayload = StudyJoinResultPayload.rejected(study.getId(), applicant.getId());
    ...
    public EventType support() {
        return EventType.STUDY_JOIN_REJECTED;
    }
}
